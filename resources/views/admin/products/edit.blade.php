                                ...
                                @if ($productImages->isNotEmpty())
                                    @foreach ($productImages as $productImage)
                                        <div class="col-md-3" id="image-row-{{ $productImage->id }}">
                                            <input type="hidden" id="image_id" name="image_array[]"
                                                value="{{ $productImage->id }}">
                                            <img src="{{ asset('/uploads/product/' . $productImage->image) }}"
                                                class="card-img-top">
                                            <a onclick="deleteImage({{ $productImage->id }})"
    ...
    <script>
        ...
        const dropzone = $('#image').dropzone({
            ...
            success: function(file, response) {
                var html =
                    `<input type="hidden" id="image_id" name="image_array[]" value="${response.image_id}">`;
                $('#prop').append(html);
                // console.log(response);
            }
        })
            ...
            if (confirm("Are you sure you want to delete")) {
                $.ajax({
                    ...
                    success: function(data) {
                        if (data.status == true) {
                            alert(data.message);
                            $('#product-gallery').load('product-gallery');
                        } else {
                            alert('bye');
                        }
                    }
